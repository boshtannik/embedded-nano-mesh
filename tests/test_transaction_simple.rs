...
mod tests {
    ...
    const NODE_2_UPDATE_PERIOD: ms = 1;
    const SIMULATION_TIMEOUT: ms = 400;

    #[test]
    fn test_send_transaction_simple() {
        ...
        let transacter_thread = std::thread::spawn(move || {
            match node_1.send_with_transaction(
        ...
        let mut is_node_2_transacted = false;
        ...
        let is_transaction_done = transacter_thread
            .join()
            .expect("Fail to join pinger thread");

        network_simulator.stop_simulation_thread();

        assert!(is_node_2_transacted);
        assert!(is_transaction_done);
    }
}
